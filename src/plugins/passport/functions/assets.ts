...
export function passportAssetDir(guildId: string): string {
  return join(ASSETS_ROOT, guildId, "passport");
}

export function passportAssetPath(guildId: string, assetId: string): string {
  const safeId = assetId.replace(/[^a-zA-Z0-9_-]/g, "");
  if (!safeId || safeId !== assetId) {
    throw new Error("Invalid asset id");
  }
  return join(passportAssetDir(guildId), `${safeId}.png`);
}

export function readPassportAsset(guildId: string, assetId: string): Buffer | null {
  try {
    const path = passportAssetPath(guildId, assetId);
...
export function deletePassportAsset(guildId: string, assetId: string): boolean {
  try {
    const path = passportAssetPath(guildId, assetId);
...
export async function savePassportBackgroundAsset(
  guildId: string,
  raw: Buffer,
  ...
  const dir = passportAssetDir(guildId);
  mkdirSync(dir, { recursive: true });
  writeFileSync(passportAssetPath(guildId, assetId), png);
