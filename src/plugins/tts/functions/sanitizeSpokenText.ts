...
// Every Discord mention shape: <@id>/<@!id> (user, incl. legacy nickname form), <@&id> (role),
// <#id> (channel), and </name:id> (slash-command mention). Left unresolved to a name on purpose —
// reading a raw snowflake, a "@Someone" ping, or a "#channel-name" out loud is worse noise than
// just dropping it.
const MENTION_PATTERN = /<@[!&]?\d+>|<#\d+>|<\/[\w-]+(?: [\w-]+)*:\d+>/g;

// Plain-text mass-ping tokens that aren't wrapped in angle brackets.
const MASS_MENTION_PATTERN = /@(everyone|here)\b/g;
...
/**
 * Strips URLs, bare media filenames (gifs, images, video clips), mentions (users, roles,
 * channels, slash-command links, and @everyone/@here), and emoji (both Discord custom emoji and
 * Unicode ones) before text is spoken — none of that reads as anything but noise out loud.
 * Returns "" if nothing meaningful is left (e.g. the whole message was just a link or a string of
 * emoji), which callers treat the same as an empty message.
 */
