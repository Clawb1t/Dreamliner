...
function sellButton(plane: PlaneTypeRow, ownerId: string, t: Translator): { button: ButtonBuilder; price: number } {
 ...
 */
export function buildCardReveal(plane: PlaneTypeRow, t: Translator, ownerId?: string): CardDisplayResult {
  const buttons = [
    new ButtonBuilder()
      .setCustomId(`${PLANE_STATS_PREFIX}${plane.id}`)
      .setLabel(t("economy.card.viewStatsButton", "View Stats"))
      .setStyle(ButtonStyle.Secondary),
  ];
  if (ownerId) buttons.push(sellButton(plane, ownerId, t).button);
...
export function buildInventoryPage(
  ...
  const buttons = [
    ...
    new ButtonBuilder()
      ...
      .setLabel(t("economy.card.backButton", "Back"))
      .setStyle(ButtonStyle.Secondary)
  ...
  if (isOwnHangar) buttons.push(sellButton(plane, opts.targetUserId, t).button);
  buttons.push(
    new ButtonBuilder()
      .setCustomId(`${PLANE_INVENTORY_PREFIX}${idBase}:n:${nextIndex}`)
      .setLabel(t("economy.card.nextButton", "Next"))
      .setStyle(ButtonStyle.Secondary)
      .setDisabled(!canScroll),
  );
...
export function buildCardRevealBatch(planes: PlaneTypeRow[], ownerId: string, t: Translator): CardDisplayResult {
  const components: TopLevelComponentData[] = [];
  const files: AttachmentBuilder[] = [];
  for (const plane of planes) {
    const reveal = buildCardReveal(plane, t, ownerId);
